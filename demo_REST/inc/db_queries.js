...
module.exports = {
    ...
    postUserRegister: function(conn, req, res) {
        if (conn) {
            ...
            var qryStr = 'INSERT into user (user_name, password, user_email) values("' + userName + '", "' + userPassword + '","' + userEmail + '")';
            executeQuery(conn, res, qryStr);
        } else {
            res.json({
                'success': false,
    ...
    postUserUpdate: function(conn, req, res) {
        if (conn) {
            ...
            var qryStr = `UPDATE user SET ${type} = '${value}' WHERE id = ${userId}`;
            executeQuery(conn, res, qryStr);
        } else {
            res.json({
                'success': false,
    ...
    postUserEmailUpdate: function(conn, req, res) {
        if (conn) {
            var userName = req.body.user_name;
            var userEmail = req.body.user_email;
            var qryStr = 'UPDATE user SET user_email = "' + userEmail + '" WHERE user_name = "' + userName + '"';
            executeQuery(conn, res, qryStr);
        } else {
            res.json({
                'success': false,
